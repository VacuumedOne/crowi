module.exports = function(crowi) {
  ...
  pageSchema.post('save', async savedPage => {
    const Backlink = crowi.model('Backlink')
    let result
    result = await Backlink.createBySavedPage(savedPage).catch(err => err)
    debug(result)
    const Mention = crowi.model('Mention')
    result = Mention.createByPage(savedPage).catch(err => err)
    debug(result)
  })
