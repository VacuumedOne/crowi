module.exports = function(crowi) {
  ...
  activitySchema.methods.getNotificationTargetUsers = async function() {
    ...
    const targetUsers = await model.getNotificationTargetUsers()
    const filteredUsers = targetUsers.filter(user => {
      if (user.toString() === actionUser.toString()) {
  ...
  activitySchema.post('save', async function(savedActivity) {
    ...
    const { target, user: actionUser } = savedActivity
    try {
      ...
      const watchers = filter(unique([...targetUsers, ...watchUsers]), [...ignoreUsers, actionUser])
      const notificationPromises = watchers.map(user => Notification.upsertByActivity(user, savedActivity))
